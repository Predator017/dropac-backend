...
const rabbitmqLogger = require('../../src/utils/logger/rabbitmqLogger');
  ...
  try {
    ...
    if (connection) {
      try {
        ...
        rabbitmqLogger.error(`Error closing existing connection: ${err.message}`);
    ...
    rabbitmqLogger.error(`Error connecting to RabbitMQ: ${error}`);
...
const reconnectRabbitMQ = async () => {
  ...
  try {
    ...
    rabbitmqLogger.error(`Failed to reconnect to RabbitMQ:, ${error}`);
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ch.sendToQueue(queueName, Buffer.from(JSON.stringify(message)), {
        ...
        if (err) {
          rabbitmqLogger.error(`Failed to publish message to ${queueName}, error: ${err}`);
          //console.error(`Failed to publish message to ${queueName}:`, err);
          reject(err);
        } else {
          rabbitmqLogger.info(`✅ Successfully published message to ${queueName}`);
    ...
    rabbitmqLogger.error(`Error publishing to queue ${queueName}: error: ${error}`);
