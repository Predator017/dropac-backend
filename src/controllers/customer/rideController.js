  ...
  try {
    ...
    if (existingRequest) {
      ...
      if (currentTime > timeoutTime) {
        ...
        existingRequest.cancelledAt = moment()
          .tz("Asia/Kolkata")
          .format("YYYY-MM-DD HH:mm:ss");
    ...
    const rideRequest = new Ride({
      ...
      timeoutAt: moment()
        .tz("Asia/Kolkata")
        .add(10, "minutes")
        .format("YYYY-MM-DD HH:mm:ss"), // 10-minute expiration
    });
    ...
    // Publish to RabbitMQ
    try {
      const queueName = outStation
        ? "outstation-ride-requests"
        : "ride-requests";
      rideLogger.info(
        `Publishing ride ${rideRequest._id} to queue ${queueName}`
      );
      ...
      if (published) {
        rideLogger.info(
          `✅ Ride request ${rideRequest._id} published to queue ${queueName} successfully`
        );
      } else {
        rideLogger.error(
          `❌ Failed to publish ride request ${rideRequest._id} to queue ${queueName}`
        );
      }
    } catch (mqError) {
      rideLogger.error(
        `Error with RabbitMQ while creating ride request: ${mqError}`
      );
      // Continue with the response even if RabbitMQ fails, as ride is saved in DB
    }

    rideLogger.info(
      `Ride request created successfully: userid: ${req.body.userId} rideid: ${rideRequest._id}`
    );
    res
      .status(201)
      .json({
    ...
    res
      .status(500)
      .json({ message: "Creating ride request failed", error: error.message });
  }
};
...
exports.cancelRideRequest = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          rideLogger.error(
            `Error cancelling consumer for ride ${rideId}: error: ${cancelError}`
          );
        }
      }, 5000);
    } catch (mqError) {
      rideLogger.error(
        `Error with RabbitMQ while cancelling ride: error: ${mqError}`
      );
    ...
    rideLogger.info(
      `Ride request cancelled successfully, userid: ${req.body.userId} rideid: ${ride._id}`
    );
    res
      .status(200)
    ...
    res
      .status(500)
      .json({ message: "Cancelling ride failed", error: error.message });
  }
};
...
exports.getRideStatus = async (req, res) => {
  ...
  try {
    ...
    if (new Date() > new Date(ride.timeoutAt) && ride.status == "pending") {
      return res
        .status(400)
        .json({ message: "No riders, please try again", ride });
    }
...
exports.rateDriver = async (req, res) => {
  ...
  try {
    ...
    driver.rating = parseFloat(driver.rating);
